...
def _fmt(amount: int) -> str:
    """Format số tiền kiểu Việt Nam: 1.450.000đ"""
...
def search_flights(origin: str, destination: str) -> str:
    ...
    """
    flights = FLIGHTS_DB.get((origin, destination)) or \
              FLIGHTS_DB.get((destination, origin))
    if not flights:
        return f"Không tìm thấy chuyến bay từ {origin} đến {destination}."
    
    lines = [f"✈️ Chuyến bay {origin} → {destination}:\n"]
...
def search_hotels(city: str, max_price_per_night: int = 99_999_999) -> str:
    ...
    if not filtered:
        budget_fmt = f"{max_price_per_night:,}đ".replace(",", ".")
        return (f"Không tìm thấy khách sạn tại {city} với giá dưới "
                f"{budget_fmt}/đêm. Hãy thử tăng ngân sách.")
    ...
    for h in filtered:
        ...
        lines.append(
            f"{'⭐' * h['stars']} {h['name']} | {price_fmt}/đêm "
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    lines.append(f"  {'─'*30}")
    lines.append(f"  Tổng chi:  {total_spent:,}đ".replace(",", "."))
    lines.append(f"  Ngân sách: {total_budget:,}đ".replace(",", "."))
    
    if remaining >= 0:
        lines.append(f"  Còn lại:   {remaining:,}đ ✅".replace(",", "."))
    else:
        lines.append(f"  Vượt ngân sách: {abs(remaining):,}đ ❌ Cần điều chỉnh!".replace(",", "."))
